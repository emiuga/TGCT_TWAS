---
title: "Single cell data (for TWAS paper): SOMATIC germ cells (males, n=22)"
...
## Output file
```{r}
file_out="../Output/Table.PERMtest_Single_Cell_somatic_TWAS_49genes_57JOINTsig_v1.0.txt"
```
...
## Permutation analysis: see: Scripts/PERMtest_SingleCell_GERMCELLS_165genes_TWASfdr01_v0.1.Rmd \
```{r}
...
- Observed values
```{r}
mean_SC_top_M <- mean_SC_top_M[levels(celltype_filt)]
mean_SC_top_M
```
...
```{r}
# Set order of rows
cell_table <- c("GermCells", "Sertoli", "FetalLeydig")
```
