---
title: "Single cell data (for TWAS paper): human_germcells annotations"
...
```{r}
nze_top <- apply(X[rownames(X)%in%twas$ID, select ], 1, function(x) tapply(x, celltype_filt, function(x) {length(x[x>0])/length(x)*100 } ))
```
...
```{r}
mean_sc_top <- apply(X[rownames(X)%in%twas$ID, select ], 1, function(x) tapply(x, celltype_filt, function(x) 
...
n_cat <- length(levels(factor(celltype_filt)))
s_size
n_cat
```
...
for(i in 1:perm) {
  ...
  nze <- apply(s, 1, function(x) tapply(x, celltype_filt, function(x) { length(x[x>0])/length(x)*100 } ))
...
save(list=c("P_nze_median", "nze_top", "nze_top_M"), file="../Data/perm10K.GERMCELLS_Proportion_expressed_49genes_57JOINTsig_v1.0.Rdat")
```
...
for(i in 1:perm) {
  s <- X_twas[ sample(nrow(X_twas), size=s_size),  ]
  ave <- apply(s, 1, function(x) tapply(x, celltype_filt, function(x) { mean( scale(x, center=F), na.rm=T) } ))
  ave[is.na(ave)] <- 0
...
save(list=c("P_ave_sc_median", "P_ave_sc_q25", "P_ave_sc_q75", "mean_SC_top_M" ), file="../Data/perm10K.GERMCELLS_ave_scaled_exp_49genes_57JOINTsig_v0.1.Rdat")
```
