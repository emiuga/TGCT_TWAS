...
suppressMessages(library("shades"))

# Modified post process FUSION (version 1.6)
# Emilio Ugalde <emilio.ugalde.morales@ki.se>
## Lambda (regularization constant used to normalize gene expression covariance) was added. Defalut=0
## Lambda reg. is implemented if more than one putative gene is found at a give locus.
...
# print scripts in use
print(paste0("Running: ", script.name))

option_list = list(
  make_option("--input", action="store", default=NA, type='character',
              help="Path to TWAS test output [required]"),
  make_option("--full_twas", action="store", default=NA, type='character',
              help="Path to full TWAS output [required]"),
  make_option("--out", action="store", default=NA, type='character',
              help="Path to output files [required]"),
  make_option("--sumstats", action="store", default=NA, type='character',
              help="Path to LDSC format summary statistics [required]"),
  make_option("--lambda", action="store", default=0, type='double',
              help="lambda regularization value for performing summary GWAS conditional analysis [default: %default]"),
  ...
  make_option("--pval_line", action="store_true", default=FALSE,
              help="plot P-value threshold line used for estimating variance explained after conditional analysis [default: OFF]"),  
  make_option("--gwas_line", action="store_true", default=FALSE,
              help="add GWAS-sig line to conditional plots [default: OFF]"),
  make_option("--gene_legend", action="store_true", default=FALSE,
              help="add gene panel [default: OFF]")                          
)
...
if ( !is.null(opt$gene_legend) ) { opt$plot_legend=NA }
...
chr = opt$chr
# print lambda
print(paste0("Using lambda (if more than one gene): ", opt$lambda))
...
# add TWAS genes
gtwas = read.table(opt$full_twas,as.is=T,head=T)
...
if ( sum(cond.z^2 > zthresh^2) == 0 ) {
	...
	while ( sum(cond.z^2 > zthresh^2) != 0 ) {
		...
		# zero-out lambda if there is less 2 genes
		if( length(cond.z[cond.z!=0]) <2 ) lambda=0 else lambda=opt$lambda
		
		if( opt$verbose > 1 ) cat( wgtlist$PANEL[ which.max(cond.z^2) ] , wgtlist$ID[ which.max(cond.z^2) ] , " added to model with conditional Z-score of ", cond.z[which.max(cond.z^2)] , "Original TWAS.Z: ", (wgtlist$TWAS.Z)[ which.max(cond.z^2) ], "\n" , sep='\t' , file=stderr() ) 

		#cur.dinv = solve(ge_g.ld[ge.keep,ge.keep])
		cur.dinv = solve(ge_g.ld[ge.keep,ge.keep] + lambda*diag(length(which(ge.keep))))
		for ( i in 1:length(cond.z) ) {
			if ( ge.keep[i] || ge.drop[i] ) {
				...
				cond.z[i] = 0
				ge.corr[i] = 1
				if( opt$verbose > 1 ) cat( wgtlist$PANEL[ i ] , wgtlist$ID[ i ] , " dropped from the model due to correlation higher than --max_r2 to other genes in the model\n" , sep='\t' , file=stderr() ) 
			} else {
				...
				if ( cur.b.var < 0 ) {
					cond.z[i] = 0
					if( opt$verbose > 1 ) cat(wgtlist$PANEL[ i ] , wgtlist$ID[ i ] , " dropped from the model due to misspecified correlation with other genes\n" , sep='\t' , file=stderr() ) 
		...
		if ( sum(cur.unstable) != 0 ) {
			if( opt$verbose > 1 ) cat( unlist( wgtlist$PANEL[ cur.unstable ] ) , unlist( wgtlist$ID[ cur.unstable ] ) , " became more significant after conditional analysis and are dropped from the model (this is usually a sign of LD mismatch or a complex locus)", "Original TWAS.Z: ", unlist( wgtlist$TWAS.Z[ cur.unstable ] ), "\n" , sep='\t' , file=stderr() ) 
	...
	joint.keep = ge.keep
	# zero-out lambda if there is less 2 genes
	if( length(which(joint.keep)) <2 ) lambda=0 else lambda=opt$lambda
	
	#cur.dinv = solve(ge_g.ld[joint.keep,joint.keep])
	cur.dinv = solve(ge_g.ld[joint.keep,joint.keep] + lambda*diag(length(which(ge.keep))))
	...
	write.table( format(df.out,digits=5) , quote=F , col.names=T , row.names=F , sep='\t' , file=paste(opt$out,".joint_included.dat",sep='') )
	...
	write.table( format(df.out,digits=5) , quote=F , col.names=T , row.names=F , sep='\t' , file=paste(opt$out,".joint_dropped.dat",sep='') )
...
if ( opt$report ) {
	...
	cat( "FILE" , "LOCUS","CHR" , "P0" , "P1" , "nSNPs", "HIT.GENES" , "JOINT.GENES" , "BEST.TWAS.P" , "BEST.SNP.P" , "COND.SNP.P" , "VAR.EXP", "mean_VAR.EXP.avg", "mean_VAR.EXP.sug", "mean_VAR.EXP.lth\n", sep='\t' , file=file.report )
}
...
for ( i in 1:length(cons.loc.starts) ) {
	...
	joint.drop = rep(F,length(ge_g.z))
	joint.corr = rep(F,length(ge_g.z))

	if ( sum(cond.z^2 > zthresh^2) == 0 ) {
		if( opt$verbose > 0 ) 
		cat( "WARNING: no models in CLUMP ", i , " had an absolute marginal association statistic higher than " , zthresh , ". Skipping\n" , sep='' ) #, file=stderr() ) 
	} else {
		while ( sum(cond.z^2 > zthresh^2) != 0 ) {
			# add most conditionally significant feature 
			joint.keep[ which.max(cond.z^2) ] = T
			if( opt$verbose > 1 ) 
			cat( (wgtlist$PANEL)[ which.max(cond.z^2) ], (wgtlist$ID)[ which.max(cond.z^2) ] , " added to model with conditional Z-score of ", cond.z[which.max(cond.z^2)] , "Original TWAS.Z: ", (wgtlist$TWAS.Z)[ which.max(cond.z^2) ], "\n", sep='\t', file=stderr() ) 

			# zero-out lambda if there is less 2 genes
			if( length(cond.z[cond.z!=0]) <2 ) lambda=0 else lambda=opt$lambda
		
			#cur.dinv = solve(ge_g.ld[joint.keep,joint.keep])
			cur.dinv = solve(ge_g.ld[joint.keep,joint.keep] + lambda*diag(length(which(joint.keep))))
			for ( ii in which(cond.z != 0) ) {
				if ( joint.keep[ii] || joint.drop[ii] ) {
					cond.z[ii] = 0
				} else if ( max(ge_g.ld[ii,joint.keep]^2) > opt$max_r2 ) {
					cond.z[ii] = 0
					joint.corr[ii] = T
					if( opt$verbose > 1 ) 
					cat( wgtlist$PANEL[ ii ] , wgtlist$ID[ ii ] , " dropped from the model due to correlation higher than --max_r2 to other genes in the model\n" , sep='\t', file=stderr() )
				} else {
					# estimate conditional effect size
					...
					if( opt$verbose > 1 ) 
					cat( wgtlist$PANEL[ ii ] , wgtlist$ID[ ii ] , " dropped from the model due to misspecified correlation with other genes\n" , sep='\t', file=stderr() ) 
			...
			if ( sum(cur.unstable) != 0 ) {
				if( opt$verbose > 1 ) 
				cat( unlist( (wgtlist$PANEL)[ cur.unstable ] ) , unlist( (wgtlist$ID)[ cur.unstable ] ) , " became more significant after conditional analysis and are dropped from the model (this is usually a sign of LD mismatch or a complex locus)", "Original TWAS.Z: ", unlist( (wgtlist$TWAS.Z)[ cur.unstable ] ), "\n" , sep='\t', file=stderr() ) 
				joint.drop[ cur.unstable ] = T
				cond.z[ cur.unstable ] = 0
				}
			}
			if( opt$verbose > 1 ) cat( "final best conditional Z^2 = " , max(cond.z^2) , "\n" , sep='' , file=stderr() ) 
	...
	# FINAL FEATURE SELECTED ESTIMATES at each locus:
	#ge.keep = ge.keep & !ge.drop
	#joint.keep = ge.keep
	joint.keep = joint.keep & !joint.drop
	
	# zero-out lambda if there is less 2 genes
	if( length(which(joint.keep)) <2 ) lambda=0 else lambda=opt$lambda
	
	cur.dinv = solve(ge_g.ld[joint.keep,joint.keep] + lambda*diag(length(which(joint.keep))))	
	not_selec=which(ge.keep)[!which(ge.keep)%in%which(joint.keep)]
	...
	joint.z = joint.b / joint.se
	joint.p = 2*(pnorm( abs( joint.z ) , lower.tail=F))
	df.out = data.frame( "FILE" = wgtlist$FILE[ which(ge.keep)[which(ge.keep)%in%which(joint.keep)] ] , "ID" = wgtlist$ID[ which(ge.keep)[which(ge.keep)%in%which(joint.keep)] ] , "TWAS.Z" = wgtlist$TWAS.Z[ which(ge.keep)[which(ge.keep)%in%which(joint.keep)] ] , "TWAS.P" = wgtlist$TWAS.P[ which(ge.keep)[which(ge.keep)%in%which(joint.keep)] ]  , "JOINT.BETA" = joint.b , "JOINT.BETA.SE" = joint.se , "JOINT.Z" = joint.z , "JOINT.P" = joint.p )
	write.table( format(df.out,digits=5) , quote=F , col.names=T , row.names=F , sep='\t' , file=paste(opt$out,".LOCI_joint_included_loc", i,".dat",sep='') )

	# conditional estimate for features dropped from the model:
	#cur.b = ge_g.z[i] - ge_g.ld[i,ge.keep,drop=F] %*% (cur.dinv %*% ge_g.z[ge.keep,drop=F])
	cond.b = ge_g.z[not_selec] - ge_g.ld[not_selec,joint.keep,drop=F] %*% (cur.dinv %*% ge_g.z[joint.keep])
	cond.b.var = diag( 1 - ge_g.ld[not_selec,joint.keep,drop=F] %*% ( cur.dinv ) %*% t(ge_g.ld[not_selec,joint.keep,drop=F]) )
	cond.b.var[ cond.b.var < 0 ] = NA
	cond.se = sqrt( cond.b.var )
	cond.z = cond.b / cond.se
	cond.p = 2*(pnorm( abs( cond.z ) , lower.tail=F))
	df.out = data.frame( "FILE" = wgtlist$FILE[ which(ge.keep)[!which(ge.keep)%in%which(joint.keep)] ] , "ID" = wgtlist$ID[ which(ge.keep)[!which(ge.keep)%in%which(joint.keep)] ] , "TWAS.Z" = wgtlist$TWAS.Z[ which(ge.keep)[!which(ge.keep)%in%which(joint.keep)] ] , "TWAS.P" = wgtlist$TWAS.P[ which(ge.keep)[!which(ge.keep)%in%which(joint.keep)] ]  , "COND.BETA" = cond.b , "COND.BETA.SE" = cond.se , "COND.Z" = cond.z , "COND.P" = cond.p )
	write.table( format(df.out,digits=5) , quote=F , col.names=T , row.names=F , sep='\t' , file=paste(opt$out,".LOCI_joint_dropped_loc", i,".dat",sep='') )
		
		
	
		# ---- GWAS-conditional estimates (used for plotting)
		...
		## add variance explained for all SNPs
		var.exp = 1 - snp.cond.z^2 / snp.z^2  
		# no. of SNPs with GWAS statistic
		snp.n = length(pv[!is.na(pv)]) 
		# pval threshold adjusting for no. of SNPs in locus
		snp.pv.thr = -log10(1/snp.n)
		
		# generate before/after manhattan plot
		if ( opt$plot ) {
			# get overlapping genes (based on genomic location)
			...
			# identify overlapping genes with TWAS-tested genes
			gtwas_bk = gtwas$ID[!gtwas$ID%in%wgtlist$ID[ge.keep] ]
			twas.glist = cur.glist[cur.glist$ID%in%gtwas_bk,]
			
			# exclude these genes from "cur.glist"
			cur.glist = cur.glist[!cur.glist$ID%in%gtwas_bk,]

			#tot = nrow(cur.glist) + sum(ge.keep)
			tot = nrow(cur.glist) + nrow(twas.glist) + sum(ge.keep)
		

			pdf( file=paste(opt$out,".loc_",i,".pdf",sep='') , height = 2 + log(tot)/2 )
			#EUM: increase top margin to allow for legend.
			#par( oma = c(5,4,0,0)+0.1 , mar=c(0,0,1,1)+0.1 , xpd=NA , las=1 )
			par( oma = c(5,4,2,1)+0.1 , mar=c(0,0,0.5,1)+0.5 , xpd=NA , las=1 )
			...
			# Gene classes 
			gene_class=ifelse(joint.keep[ ge.keep ], "Leading", ifelse(joint.drop[ ge.keep ], "Dropped", ifelse(joint.corr[ ge.keep ], "Correlated", "Conditional") ) )
			gene_class_uniq=unique(gene_class)
			# Gene classes present in locus
			gene_t=c("Leading", "Conditional", "Correlated", "Dropped")
			gene_in=which(gene_t%in%gene_class_uniq)
			
			# Gene classes: colors
			## For filling:
			gene_color=brewer.pal( length(gene_t) , "Paired" )
			## For border:
			gene_color_contrast=brightness(gene_color, 0.75)
			...
			if ( opt$plot_legend == "all" ) {
				...
				clr.leg = c( rep(NA,nrow(cur.glist)+nrow(twas.glist)) , clr.ref[ m ] )
				clr.num = c( rep("",nrow(cur.glist)+nrow(twas.glist)) , m )
				...
				clr.leg = c( rep(NA,nrow(cur.glist)+nrow(twas.glist)) , clr.leg )
				clr.num = c( rep("",nrow(cur.glist)+nrow(twas.glist)) , clr.num )		
			}
			}
			# background genes ("Not tested")
			clr.fg = rep("gray80",nrow(cur.glist))
			clr.bg = rep("white",nrow(cur.glist))
			# background TWAS genes ("Marginally associated genes")
			clr.ft = rep("gray60",nrow(twas.glist))
			clr.bt = rep("white",nrow(twas.glist))
			# TWAS genes (initiate as "conditional")
			clr.fg2 = rep(gene_color_contrast[which(gene_t=="Conditional")], sum(ge.keep) ) 
			clr.bg2 = rep(gene_color[which(gene_t=="Conditional")], sum(ge.keep) ) 
			# edit color for <<dropped genes>> (became more significant in cond. analysis)
			clr.fg2[ joint.drop[ ge.keep ] ] = gene_color_contrast[which(gene_t=="Dropped")] 
			clr.bg2[ joint.drop[ ge.keep ] ] = gene_color[which(gene_t=="Dropped")]
			# edit color for <<correlated genes>> (became more significant in cond. analysis)
			clr.fg2[ joint.corr[ ge.keep ] ] = gene_color_contrast[which(gene_t=="Correlated")]
			clr.bg2[ joint.corr[ ge.keep ] ] = gene_color[which(gene_t=="Correlated")]
			
			# selected "Leading" genes
			clr.fg2[ joint.keep[ ge.keep ] ] = gene_color_contrast[which(gene_t=="Leading")] 
			clr.bg2[ joint.keep[ ge.keep ] ] = gene_color[which(gene_t=="Leading")]
						
			# combine
			#clr.text = c( clr.fg , clr.ft, clr.drop )
			clr.fg = c( clr.fg , clr.ft, clr.fg2 )
			clr.bg = c( clr.bg , clr.bt, clr.bg2 )
				
			cur.glist = rbind( cur.glist , twas.glist, wgtlist[ ge.keep , c("CHR","P0","P1","ID") ] )	
		
			ord = order(apply(cur.glist[,2:3],1,min))
			cur.glist = cur.glist[ ord , ]	
			clr.fg = clr.fg[ ord ]
			clr.bg = clr.bg[ ord ]
			#clr.text = clr.text[ ord ]
			if ( !is.na( opt$plot_legend) ) {
				clr.leg = clr.leg[ ord ]
				clr.num = clr.num[ ord ]
			}
			...
			while ( TRUE ) {
				...
				if ( g.ctr > 1 ) {
					...
					plot( 0 , 0 , type="n" , ylim = c(-0.1,1.1) , xlim = range( genos$bim[ cur.keep , 4 ] / 1e6 ) , bty="n" , xlab="" , ylab="" , xaxt="n" , yaxt="n" )
					# background
					#clr.fg = rep("gray",nrow(cur.glist))
					#clr.bg = rep("gray",nrow(cur.glist))
					#clr.fg[ cur.glist$ID == wgtlist$ID[ cur.gene ] ] = "#009E73"
					#clr.bg[ cur.glist$ID == wgtlist$ID[ cur.gene ] ] = "#00c690"		
				}
				...
				if ( !is.na( opt$plot_legend) ) {
					...
					legend( "topleft" , legend=paste(1:(length(uni.ref.names)),uni.ref.names), pch=22 , cex=txt.scale , col=clr.ref , bty="n" )
				}
				
				# Gene legend
				if ( !is.na( opt$gene_legend) ) {
				    gene_text=append(gene_t[gene_in], c("Marginally associated", "Not tested"))
				    gene_color_fill=append(gene_color[gene_in], c("white", "white"))
				    gene_color_border=append(gene_color_contrast[gene_in], c("gray60", "gray80"))
				    # To make border in white:
				    #gene_color_border=rep("white", length(gene_color_fill) )
				    legend("top", legend=gene_text, inset=c(0,-0.2),fill=gene_color_fill, border=gene_color_border, bty="n", cex=0.8, horiz=T  )			   
				}		
				...
				if ( opt$plot_eqtl && ( sum(ge.keep) == 1 || g.ctr > 1 ) ) {
					...
					plot( genos$bim[ cur.keep , 4 ] / 1e6 , -log10(pv) , pch=19 , cex=0.5 , col="gray" , xlab=paste("chr ", chr , " physical position (MB)",sep='') , ylab="-log10(P-value)" , bty="n", ylim=c() )
					
				
				# add horizontal line with var.exp Pval threshold (adjusted to no. of SNPs in locus)
				clip(min(genos$bim[ cur.keep , 4 ] / 1e6 ), max( genos$bim[ cur.keep , 4 ] / 1e6 ), 0, max(-log10(pv), na.rm=T)+0.5 )
				if (opt$pval_line) abline(h=snp.pv.thr, lty=2, col="gray45")
				if (opt$gwas_line) 
				if(max(-log10(pv), na.rm=T) > -log10(5e-8) ) abline(h=-log10(5e-8), lty=2, col="darkgrey")
				...
				points( genos$bim[ cur.keep , 4 ] / 1e6 , -log10(pv.cond) , pch=19 , cex=0.5 , col="#009E73" )
				
								
				## EUM: add legend
				# Get vector of leading genes
				lead_gene=wgtlist$ID[ joint.keep & ge.keep ]
				# Format text of genes to be printed
				if( length(lead_gene)>1 ) {
					genes_minus1 <- rep(NA, length(lead_gene)-1) 
					if( length(genes_minus1)==1 ) { 
						lead_gene <- paste0(lead_gene[1], " and ", lead_gene[2])
					 } else {
					 	x <- lead_gene[1]
					 	for(g in 2:length(genes_minus1) ) { x <- paste(x, lead_gene[g], sep=", ") }
					 	lead_gene <- paste0(x, " and ", lead_gene[ length(lead_gene) ])
				 } }
							
				gwas_text=c("GWAS", paste0("GWAS conditioned on predicted ", lead_gene))
				gwas_l_x=min(genos$bim[ cur.keep , 4 ] / 1e6 )
				gwas_l_y=max(-log10(pv), na.rm=T)
				#cat(gwas_l_x)
				#cat(gwas_l_y)
				
				# legend("left", legend=gwas_text, pch=c(19, 19), col=c("darkgrey", "#009E73"), text.col="gray35" ,  bty="n", cex=0.8)
				legend(gwas_l_x , gwas_l_y, legend=gwas_text, pch=c(19, 19), col=c("darkgrey", "#009E73"), text.col="gray25" ,  bty="n", cex=0.75)
		...
		if ( opt$save_loci ) {
			...
			write.table( format(df.out,digits=5) , quote=F , col.names=T , row.names=F , sep='\t' , file=paste(opt$out,".loc_",i,".cond",sep='') )
		}
		if ( opt$report ) {
			df.out = data.frame( "SNP" = genos$bim[cur.keep,2] , "POS" = genos$bim[cur.keep,4] , "GWAS.LOGP" = -log10(pv) , "GWAS_cond.LOGP" = -log10(pv.cond) )
			write.table( format(df.out,digits=5,trim=T) , quote=F , col.names=T , row.names=F , sep=',' , file=paste(opt$out,".loc_",i,".cond.csv",sep='') )
		}
		...
		if ( opt$report ) {
			...
			# average var.exp (SNP above average pv)
			var.exp_avg = mean(var.exp[which(-log10(pv) > mean(-log10(pv), na.rm=T))])
			# average var.exp (SNP above GWAS-sug thershold: 1e-5)
			var.exp_sug = mean(var.exp[which(-log10(pv) > 5 ) ])
			# average var.exp (SNP above "relaxed" threshold adjusted to no. of SNPs in the locus)
			var.exp_lth = mean(var.exp[which(-log10(pv) > -log10(1/snp.n ) ) ])
			cat( paste(opt$out,".loc_",i,sep='') , locus, opt$chr , range( genos$bim[ cur.keep , 4 ] ) , snp.n, length(unique(wgtlist$ID[ge.keep])) , sum( joint.keep[ ge.keep ] ) , min(wgtlist$TWAS.P[ ge.keep ],na.rm=T) , 2*pnorm(sqrt(best.snp.chisq), lower.tail=F) , 2*pnorm(sqrt(cond.snp.chisq), lower.tail=F) , 1 - cond.snp.chisq / best.snp.chisq , var.exp_avg, var.exp_sug, var.exp_lth,'\n' , sep='\t' , file=file.report , append=T )
		
			cur.wgt = wgtlist[ ge.keep , ]
			cur.wgt$JOINT = joint.keep[ ge.keep ]
			cur.wgt$JOINT_status = ifelse(joint.keep[ ge.keep ], "joint", ifelse(joint.drop[ ge.keep ], "dropped", ifelse(joint.corr[ ge.keep ], "correlated", "conditional") ) )
			if ( length(which(cur.wgt$JOINT_status=="joint")) > 1)
			cur.wgt$JOINT_status = ifelse(cur.wgt$JOINT_status=="joint" & wgtlist$ID[ge.keep]==wgtlist$ID[ge.keep][which.min(wgtlist$TWAS.P[ge.keep])], "joint_best",
					ifelse(cur.wgt$JOINT_status=="joint" & wgtlist$ID[ge.keep]!=wgtlist$ID[ge.keep][which.min(wgtlist$TWAS.P[ge.keep])], "joint_other" ,cur.wgt$JOINT_status))
			if ( length(which(joint.keep))==1 ) cur.wgt$JOINT_status[cur.wgt$JOINT_status=="joint"] = "joint_best"
			#if ( length(which(joint.keep))==1 & length(which(cur.wgt$JOINT_status=="joint"))==1 ) cur.wgt$JOINT_status = "joint_best"
			if ( length(which(ge.keep))==1 ) cur.wgt$JOINT_status = "single"
