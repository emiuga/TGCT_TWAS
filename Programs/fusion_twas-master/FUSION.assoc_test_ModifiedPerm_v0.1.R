#' FUSION.assoc_test_ModifiedPerm.R
#' 
#' Modified version of FUSION.assoc_test.R, see https://github.com/gusevlab/fusion_twas
#' 
#' Removed the abbreviated permutation option, added an external seed-argument, and 
#' fixed the calculation of the final p-value, see comments signed AP
#' 
#' Alexander.Ploner@ki.se 2022-10-17
...
option_list = list(
  ...
  ## AP: define seed option for replicability
  make_option("--rngseed", action="store", default=NA, type='integer',
...
## AP: set the random seed (could be moved closer to permutation test, probably)
set.seed(opt$rngseed)
print(paste0("Using random seed (for gene-permutation test): ", opt$rngseed))
			...
			if ( cur.twasr2pred > 0 ) {
				...
				if ( !is.na(opt$perm) && opt$perm > 0 && cur.twas^2 > permz^2 ) {
					...
					for ( i in 1:opt$perm ) {
						...
						# AP - commented this out, time savings not required here
					...
					if ( is.na(perm.pval) ) {
					  # AP: minimal p-value under null is 1/(#perm + 1), as the observed
					  # test statistics is also counted as "under the null"
					  # This is the upper bound p_u from https://arxiv.org/pdf/1603.05766
						perm.pval = ( sum(perm.twas^2 >= cur.twas[1]^2) + 1 ) / ( opt$perm + 1 )
						perm.N = opt$perm
